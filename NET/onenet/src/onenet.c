...
#define PROID			"mMCK6UJtcU"
#define ACCESS_KEY		"aUhMTHVIMjJJUUJUMmpqdjdEdEYzcHhSbDVUSjcxQjg="
#define DEVICE_NAME		"YG1"

//extern u8 temp, humi;
extern float PH_Data,TDS_Data;
extern float water_temp;
extern u8 environment_humi;
static unsigned char Heater_Status = 0;
char devid[16];

char key[48];

// [新增] 用于记录舵机当前状态，方便上传给云端同步 (0:关, 1:开)
static unsigned char Servo_Status = 0; 
...
unsigned char OneNet_FillBuf(char *buf)
{
    ...
    // 1. 初始化 JSON 头部
    snprintf(buf, 256, "{\"id\":\"%hu\",\"params\":{", ++g_msg_id);
    
    // 2. PH (float)
    sprintf(text, "\"PH\":{\"value\":%.1f},", PH_Data);
    strcat(buf, text);

    // 3. TDS (强转为整型上传)
    sprintf(text, "\"TDS\":{\"value\":%d},", (int)TDS_Data);
    strcat(buf, text);
    
    // 4. 水温 (float)
    sprintf(text, "\"WaterTemperature\":{\"value\":%.1f},", water_temp);
    strcat(buf, text);
   
    // 5. 照明开关 (bool)
    sprintf(text, "\"LightSwitch\":{\"value\":%s},", relay2_info.Relay2_Status ? "true" : "false");
    strcat(buf, text);
		
    // 6. 环境湿度 
    sprintf(text, "\"EnvHumidity\":{\"value\":%.1f},", (float)environment_humi); //
    strcat(buf, text);

    // 7. 舵机开关 
   sprintf(text, "\"ServoSwitch\":{\"value\":%s},", Servo_Status ? "true" : "false");
    strcat(buf, text);
		
    // 加热开关状态上报
    // 【修正】去掉 %s}, 最后的逗号
    sprintf(text, "\"HeaterSwitch\":{\"value\":%s}", Heater_Status ? "true" : "false");
    strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
    ...
    short result = 0;
    cJSON *raw_json = NULL, *params_json = NULL;
    
    // 定义各属性的 JSON 指针
    cJSON *led_json = NULL, *purifier_json = NULL, *pump_json = NULL;
    cJSON *servo_json = NULL;
    cJSON *heater_json = NULL; // [新增] 加热器 JSON 指针

    // 1. 判断 MQTT 报文类型
    type = MQTT_UnPacketRecv(cmd);
    switch(type)
    {
        case MQTT_PKT_PUBLISH: // 接收到平台下发的 Publish 消息（属性设置）
            ...
            if(result == 0)
            {
                ...
                if(raw_json != NULL)
                {
                    ...
                    if(params_json != NULL)
                    {
                        // --- [新增控制] 1. 处理加热器 (HeaterSwitch) ---
                        // 逻辑：控制 PA6 引脚高低电平
                        heater_json = cJSON_GetObjectItem(params_json, "HeaterSwitch");
                        if(heater_json != NULL)
                        {
                            // cJSON 中 bool 类型表现为 valueint(1/0) 或 type(cJSON_True/False)
                            if(heater_json->valueint == 1 || heater_json->type == cJSON_True) 
                            {
                                GPIO_SetBits(GPIOA, GPIO_Pin_6);  // PA6 置高电平，开启继电器
                                Heater_Status = 1;                // 更新全局状态变量（用于上报同步）
                                UsartPrintf(USART_DEBUG, "Heater ON (PA6 HIGH)\r\n");
                            }
                            else
                            {
                                GPIO_ResetBits(GPIOA, GPIO_Pin_6); // PA6 置低电平，关闭继电器
                                Heater_Status = 0;                 // 更新全局状态变量
                                UsartPrintf(USART_DEBUG, "Heater OFF (PA6 LOW)\r\n");
                            }
                        }

                        // --- 2. 处理舵机 (ServoSwitch) ---
                        servo_json = cJSON_GetObjectItem(params_json, "ServoSwitch");
                        if(servo_json != NULL)
                        {
                            if(servo_json->valueint == 1 || servo_json->type == cJSON_True) 
                            {
                                Servo_SetAngle(180); // 开启：转到 180 度
                                Servo_Status = 1;    
                                UsartPrintf(USART_DEBUG, "Servo ON (180)\r\n");
                            }
                            else
                            {
                                Servo_SetAngle(0);   // 关闭：转到 0 度
                                Servo_Status = 0;    
                                UsartPrintf(USART_DEBUG, "Servo OFF (0)\r\n");
                            }
                        }
                        ...
                        if(led_json != NULL)
                        {
                            ...
                            DelayXms(10);                      
                            GPIO_ResetBits(GPIOB, GPIO_Pin_13); // 触发下降沿
                            DelayXms(10);                      
                            GPIO_SetBits(GPIOB, GPIO_Pin_13);  
                            UsartPrintf(USART_DEBUG, "LED Triggered\r\n");
                        }
                        ...
                        if(pump_json != NULL)
                        {
                            ...
                            DelayXms(10);                      
                            GPIO_SetBits(GPIOB, GPIO_Pin_14);  
                            UsartPrintf(USART_DEBUG, "Pump Triggered\r\n");
                        }

                        // --- 5. 处理净水器 (Purifier) ---
                        purifier_json = cJSON_GetObjectItem(params_json, "purifier");
                        if(purifier_json != NULL)
                        {
                            ...
                            DelayXms(10);                      
                            GPIO_SetBits(GPIOB, GPIO_Pin_12);  
                            UsartPrintf(USART_DEBUG, "Purifier Triggered\r\n");
                        }
                    }
                    // 3. 释放 cJSON 内存
        ...
        case MQTT_PKT_PUBACK:
            UsartPrintf(USART_DEBUG, "Tips: Device Publish Ack\r\n");
            break;
        case MQTT_PKT_SUBACK:
            UsartPrintf(USART_DEBUG, "Tips: Subscribe Success\r\n");
            break;
        default:
            break;
    }
    ...
    // 4. 清理工作
    ESP8266_Clear(); // 清空 ESP8266 接收缓存
    if(type == MQTT_PKT_PUBLISH)
    {
        MQTT_FreeBuffer(cmdid_topic); // 释放 MQTT 库生成的内存
